...
const DOCUMENTATION_MAP_CENTER: [number, number] = [27.2, 17.2];
...
export default function DocumentationTeam() {
  ...
  const persistedLayers = trpc.atlas.layers.useQuery();

  const customLayers = persistedLayers.data ?? [];
  const availableLayers = useMemo(() => [...documentationLayers, ...customLayers], [customLayers]);
  ...
  const teamRole = teamAccess.data?.member?.teamRole;
  const canEdit = isAdmin || teamRole === "editor";
  const canReview = isAdmin || teamRole === "reviewer";
  const canImport = isAdmin || teamRole === "import_manager";
  const hasDocumentationAccess = isAdmin || Boolean(teamAccess.data?.member?.status === "active");
  const queue = trpc.atlas.reviewQueue.useQuery({ recordStatus: status }, { enabled: hasDocumentationAccess });
  const importJobs = trpc.atlas.importJobs.useQuery(undefined, { enabled: hasDocumentationAccess && (user?.role === "admin" || teamAccess.data?.member?.teamRole === "import_manager") });
  ...
  const duplicateQuery = trpc.atlas.findDuplicates.useQuery({ name: form.name || "غير محدد", latitude: Number(form.latitude) || 0, longitude: Number(form.longitude) || 0 }, { enabled: form.name.length > 2 && Boolean(form.latitude) && Boolean(form.longitude) && canReview });
