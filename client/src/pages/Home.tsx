    ...
    <main dir="rtl" className="atlas-shell">
      ...
      <section className="atlas-main">
        <aside className="control-panel">
          ...
          {!isPublicRoute && !isAuthenticated && <Button className="add-point-button" onClick={startLogin}><MapPinPlus size={16} /> دخول فريق التوثيق</Button>}{isAuthenticated && user?.role === "admin" && <div className="flex flex-wrap gap-2"><Button className="add-point-button" onClick={() => setEditorOpen(true)}><MapPinPlus size={16} /> إضافة نقطة إلى الأطلس</Button><Button className="add-point-button" variant="outline" onClick={() => { window.location.href = `${import.meta.env.BASE_URL}management`; }}><ShieldCheck size={16} /> مسؤول النظام</Button></div>}
