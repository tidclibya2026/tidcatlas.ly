...
import { extractKmlImageUrl, toDisplayImageUrl } from "@shared/kmlImage";
...
function parseKml(text: string, layerId: string): Site[] {
  ...
  return Array.from(xml.querySelectorAll("Placemark")).map((node, index) => {
    ...
    const rawDescription = [descriptionNode?.textContent, descriptionNode?.innerHTML, node.outerHTML].filter(Boolean).join(" ");
