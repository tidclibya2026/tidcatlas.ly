    ...
    <main dir="rtl" className="atlas-shell">
      ...
      <section className="atlas-main">
        <aside className="control-panel">
          ...
          <div className="ai-panel"><div className="ai-panel-heading"><span><Sparkles size={14} /> البحث الذكي الموثق</span><select value={assistantMode} onChange={(event) => setAssistantMode(event.target.value as typeof assistantMode)}><option value="visitor">زائر</option><option value="tourist">سائح</option><option value="researcher">باحث</option></select></div><p>مساعد معرفي يجيب من السجلات المنشورة فقط، ويعرض المواقع المطابقة ومصادرها وحدود الثقة.</p>{mapViewport && <div className="nearby-context"><span><MapPinned size={13} /> النطاق الحالي · {nearbySites.length.toLocaleString("ar-LY")} نقاط قريبة</span>{nearbySites.slice(0, 4).map((site) => <button type="button" key={site.id} onClick={() => { setSelected(site); focusSite(site, Math.max(12, mapViewport.zoom + 2)); }} title={`تبعد نحو ${site.distanceKm.toFixed(1)} كم عن مركز العرض`}>{site.name}<small>{site.distanceKm < 1 ? "أقل من 1 كم" : `${Math.round(site.distanceKm).toLocaleString("ar-LY")} كم`}</small></button>)}</div>}<div className="ai-quick-prompts" aria-label="أسئلة مقترحة"><button type="button" onClick={() => setAssistantQuestion("ما أبرز المواقع الأثرية المناسبة للزيارة؟")}>أبرز المواقع الأثرية</button><button type="button" onClick={() => setAssistantQuestion("ما أفضل مسار يجمع الطبيعة والتراث؟")}>مسار طبيعة وتراث</button><button type="button" onClick={() => setAssistantQuestion("ما المواقع المناسبة للباحثين؟")}>وضع الباحث</button><button type="button" onClick={() => setAssistantQuestion("ما نقاط الاهتمام القريبة من النطاق الظاهر حاليًا؟")}>القريب من الخريطة</button></div><div className="ai-question"><Input value={assistantQuestion} onChange={(event) => setAssistantQuestion(event.target.value)} placeholder="مثال: ما المواقع الأثرية المناسبة لزيارة قصيرة؟" aria-label="سؤال البحث الذكي" /><Button size="sm" disabled={!assistantQuestion.trim() || smartSearch.isPending} onClick={() => smartSearch.mutate({ question: assistantQuestion, mode: assistantMode, viewport: mapViewport || undefined, nearbyIds: nearbySites.map((site) => site.id), nearbyNames: nearbySites.map((site) => site.name) })}>{smartSearch.isPending ? "يبحث..." : "اسأل"}</Button></div>{smartSearch.error && <div className="ai-answer"><strong>تعذر إكمال البحث الذكي حاليًا.</strong><small>يمكنك تضييق الفلاتر أو المحاولة مرة أخرى. لا يتم عرض معلومات غير موثقة.</small></div>}{smartSearch.data && <div className="ai-answer"><div className="ai-answer-meta"><span className={`ai-confidence ai-confidence-${smartSearch.data.confidence}`}>{smartSearch.data.confidence === "high" ? "ثقة مرتفعة" : smartSearch.data.confidence === "medium" ? "ثقة متوسطة" : "ثقة محدودة"}</span><span>مصدرها سجلات الأطلس المنشورة</span></div><strong>{smartSearch.data.answer}</strong>{smartSearch.data.matchedIds.length > 0 && <small>السجلات المطابقة: {smartSearch.data.matchedIds.map((id) => activeSites.find((site) => site.id === id)?.name).filter(Boolean).join("، ")}</small>}{smartSearch.data.sources.length > 0 && <small>المصادر: {smartSearch.data.sources.join("، ")}</small>}{smartSearch.data.limitation && <small>{smartSearch.data.limitation}</small>}</div>}</div>
